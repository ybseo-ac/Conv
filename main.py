...
import diffusion
import utils
from finetune_tools import get_dataloaders_finetune
...
# 이 코드는 train 에서는 load_from_checkpoint 상황에 대한 가정이 없음
...
def generate_samples(config, logger, tokenizer): # 여기로 들어옴
  ...
  for _ in range(config.sampling.num_sample_batches): 
    if config.sampling.semi_ar: # false 가 디폴트
...
def _train(config, logger, tokenizer):
  ...
  trainer = hydra.utils.instantiate(  # instantiate -> hydera/_internal/instantiate/_instantiate2.py:148 - def instantiate()
    config.trainer,  # 'lightning.Trainer'
    default_root_dir=os.getcwd(),
    callbacks=callbacks,
    strategy=hydra.utils.instantiate(config.strategy),
    logger=wandb_logger)
...
def main(config):
  ...
  # config.original_cwd = original_cwd  #cppm
